...
function createClient (opts, cache_key) {
    ...
    if(_.isString(opts.options.password)){
        debug('sending auth command to redis');
        client.auth(opts.options.password, function(err){
            if(err)
                debug('redis auth failed for %s', opts.name);
            debug('auth Successful for %s', opts.name);
        });
    }
