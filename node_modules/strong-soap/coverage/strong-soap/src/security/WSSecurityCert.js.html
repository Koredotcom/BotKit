...
<html lang="en">
<head>
    <title>Code coverage report for strong-soap/src/security/WSSecurityCert.js</title>
    <meta charset="utf-8" />
...
<div class='wrapper'>
  <div class='pad1'>
    <h1>
      <a href="../../../index.html">All files</a> / <a href="index.html">strong-soap/src/security</a> WSSecurityCert.js
    </h1>
    <div class='clearfix'>
...
&nbsp;
  getSigningKey(privatePEM, password) {
    <span class="missing-if-branch" title="if path not taken" >I</span>if (typeof crypto.createPrivateKey === 'function') {
      // Node 11 or above
<span class="cstat-no" title="statement not covered" >      this.privateKey = crypto.createPrivateKey({key: privatePEM, passphrase: password});</span>
<span class="cstat-no" title="statement not covered" >      return this.privateKey.export({type: 'pkcs1', format: 'pem'});</span>
    } else {
      // Node 10 or below, fall back to https://github.com/rzcoder/node-rsa
      <span class="missing-if-branch" title="if path not taken" >I</span>if (password) <span class="cstat-no" title="statement not covered" >throw new Error('Passphrase is not supported by node-rsa.');</span>
